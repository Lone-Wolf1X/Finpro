...
export default function IPOApplicationAdminList() {
    ...
    const handleApprove = async (id: number) => {
        try {
            await ipoApplicationApi.approve(id, user?.firstName + ' ' + user?.lastName || 'Admin');
        ...
        <div className="p-6 w-full space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between bg-white p-6 rounded-3xl shadow-sm border border-gray-100 gap-4">
                <div>
                    ...
                    <p className="text-gray-500 font-medium">Verify, Approve, and Allot shares</p>
                </div>
                <div className="flex flex-wrap gap-2">
                    {['ALL', 'PENDING_VERIFICATION', 'PENDING', 'APPROVED', 'REJECTED', 'ALLOTTED'].map(status => (
                        <button
                            key={status}
                                    ...
                                    <tr key={app.id} className="hover:bg-gray-50/50 transition-all group">
                                        ...
                                        <td className="px-8 py-5 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                {/* VERIFY ACTION (PENDING_VERIFICATION) */}
                                                {app.applicationStatus === 'PENDING_VERIFICATION' && (
                                                    <button
                                                ...
                                                {/* ALLOT ACTION (APPROVED) */}
                                                {app.applicationStatus === 'APPROVED' && (
                                                    <button
                                                        onClick={() => {
                                                            setSelectedApp(app);
                                                            setAllotmentQty(app.quantity); // Default to full allotment
                                                        }}
                                                ...
                                                {/* REJECT ACTION (Visible for PENDING_VERIFICATION, PENDING) */}
                                                {['PENDING_VERIFICATION', 'PENDING'].includes(app.applicationStatus) && (
                                                    <button
                                                        onClick={() => {
                                                            setSelectedApp(app);
                                                            setRejectionReason('');
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="p-8 border-b border-gray-50 flex justify-between items-center">
                            <h3 className="text-xl font-black text-gray-900">
                                {selectedApp.applicationStatus === 'APPROVED' ? 'Allot Shares' : 'Reject Application'}
                            </h3>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex gap-3">
                                        ...
                                        <p className="text-[10px] font-bold text-blue-800 leading-relaxed uppercase">
                                            Allotting shares will update customer portfolio and adjust ledger balances.
                                        </p>
                                    </div>
                                    <button
